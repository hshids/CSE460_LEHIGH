...
void loop() {
  ...
  float fx = controls[1]; // Fx (foward backwards [-x,x])
  float fz = controls[2]; // Fz (height in meters [0:10])
  float tx = controls[3]; // tx
  float tz = controls[4]; // tz (compass heading in radians [0,2PI])
  ...
  float yaw_error = atan2(sin(tz-yaw), cos(tz-yaw));
  float t = PDterms.kpyaw * (yaw_error) + PDterms.kdyaw * (0 - yawrate); 
  
  float fx1 = (fx/2) - (t/2);
  float fz1 = fz/2;
  float m = sqrt(fx1*fx1 + fz1*fz1);
  
  float degree_f = constrain(-degrees(atan2(fz1,fx1)) + 45, 45, 135);
  float degree = constrain(-t + 90, 45, 135);
